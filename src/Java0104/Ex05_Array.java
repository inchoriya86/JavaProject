...
public class Ex05_Array {
	...
	public static void main(String[] args) {
		...
		num1 = new int[] {1,2,3,4,5,6,7,8};
		System.out.println("첫번째 방 : " + num1[0]);
		System.out.println("두번째 방 : " + num1[1]);
		System.out.println("일곱번째 방 : "+ num1[6]);
		System.out.println();
		
		// 배열을 선언하면서 값을 주기
		int num3[] = {10,20,30,40};
		System.out.println("1번방, index(0) : " + num3[0]);
		System.out.println("4번방, index(3) : " + num3[3]);
		...
		for(int i=0; i<scores.length; i++) { // i = 0 ~ 3 (4번 반복)
			// scores.length = 4
			// i: 인덱스 번호, scores[i] : 배열값
			System.out.println("scores[" + i +"] = " + scores[i]);	
			
			sum += scores[i];
		}
		
		// Q2.배열에 저장된 값의 총점과 평균을 구하시오! (평균 구할 시 scores.length 사용)
